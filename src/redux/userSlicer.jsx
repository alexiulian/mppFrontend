...
export const fetchUserList = createAsyncThunk('fetchUserList', async () => {
    ...
    try {
        const response = await fetch('https://mppbackend-production.up.railway.app/userList', {
            headers: {
...
export const deleteUser = createAsyncThunk('user/deleteUser', async (userId, { rejectWithValue }) => {
    const token = getToken();
    try {
        const response = await fetch(`https://mppbackend-production.up.railway.app/user/${userId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`
            }
...
export const fetchUserDetails = createAsyncThunk('fetchUserDetails', async (userId, { rejectWithValue }) => {
    const token = getToken();
    try {
        const response = await fetch(`https://mppbackend-production.up.railway.app/user?id=${userId}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
