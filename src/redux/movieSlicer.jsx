...
export const fetchMovieList = createAsyncThunk('fetchMovieList', async (_, { rejectWithValue }) => {
    ...
    try {
        const response = await fetch('https://mppbackend-production.up.railway.app/movieList', {
            headers: {
...
export const deleteMovie = createAsyncThunk('movie/deleteMovie', async (movieId, { rejectWithValue }) => {
    const token = getToken();
    try {
        const response = await fetch(`https://mppbackend-production.up.railway.app/movie/${movieId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`
            }
...
export const fetchMovieDetails = createAsyncThunk('fetchMovieDetails', async (movieId, { rejectWithValue }) => {
    const token = getToken();
    try {
        const response = await fetch(`https://mppbackend-production.up.railway.app/movie?id=${movieId}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
